...
class Graph
{
	...
	Vector<Set<size_t>> _neighbors; /**< _neighbors[i] contains the indices of the neighbors of _vertices[i] */
	...
	void addEdge(V from, V to)
		{
			...
			_neighbors[i].insert(j);
			_edgeCount++;
		}

	/* Returns true if an edge exists between the two vertices */
	bool edgeExists(V from, V to)
		{
			size_t i = _indices.at(from);
			size_t j = _indices.at(to);

			return _neighbors[i].find(j) != _neighbors[i].end();
		}
